...
def vis1(adj_list, start, curr_d, d):
    ...
    # Create the graph from the result of Functionality1        
    graph = nx.Graph()
    ...
    nx.draw(graph, pos, node_color='lightgreen', node_size = size_node, node_shape = shape_node,
            edge_color = color_edge, with_labels = True, font_size = size_font)
    gstart = nx.Graph()
    gstart.add_node(start)
    nx.draw(gstart,pos,node_color='yellow', node_size = size_node, node_shape = shape_node)
